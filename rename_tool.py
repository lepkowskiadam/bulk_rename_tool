...
class RenameTool:
    ...
    def rename(self, path, pattern, new_pattern):
        ...
        self.snapshot.save_state(path, pattern, new_pattern, 'rename')
        return updated

    def undo(self):
        self.snapshot.undo()
        path, new_pattern, old_pattern, func = self.snapshot.return_state()
        return getattr(self, func)(path, old_pattern, new_pattern)
